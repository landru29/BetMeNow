...
(function() {
  ...
  function crudRouteProvider($routeProvider) {
    ...
    this.routesFor = function(resourceName, urlPrefix, routePrefix) {
      ...
      var templateUrl = function(operation) {
        return '/public/' + baseUrl + '/views/' + operation.toLowerCase() + '.html';
      };
      ...
      var routeBuilder = {
        ...
        // Create a route that will handle editing an existing item
        whenShow: function(resolveFns) {
          routeBuilder.when(baseRoute + '/show/:itemId', {
            templateUrl: templateUrl('Show'),
            controller: controllerName('Show'),
            resolve: resolveFns
          });
          return routeBuilder;
        },
  ...
  angular.module('wcb.system.crudRouteProvider', ['ngRoute']).provider('crudRoute', crudRouteProvider);
})();
